...
public class SalonchikClient implements SalonchikController {
    ...
    protected void onTableViewSelect() {
        try {
            // Получаем выбранный элемент из таблицы и приводим его к типу DatabaseOrder
    ...
    // Метод fillUserData заполняет данные пользователя в пользовательском интерфейсе
	...
	public ObservableList retrieveUserOrders(int userId) {
		...
		try {
            ...
            if (userId > 0) {
                // Получаем список заказов пользователя и преобразуем его в ObservableList
				List<DatabaseOrder> orders = this.database.clientOrders(userId);
				return FXCollections.observableList(orders);
            }
            // Если идентификатор пользователя меньше или равен 0, выбрасываем исключение
            throw new NullPointerException("Невозможно получить список заказов для данного пользователя!");
        } catch (Exception error) {
    ...
    public void onCreateButtonClick(ActionEvent actionEvent) {
        // Попытка открытия окна создания нового заказа для текущего пользователя
        try {
